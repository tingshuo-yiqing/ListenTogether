...
全员离线且宽限到期时，tick 将 hostId 清为空串（现有协议允许），不再引用已删除成员；重复 tick 不重复产生转移事件。没有现存房主时，connect 在第一份在线快照广播前选最早加入的在线成员，消除最多 250ms 的无房主权限窗口。仍在宽限中的房主不被抢占。新增 3 项可控时钟回归，后端共 23 项；本地已修复，未部署云端。
