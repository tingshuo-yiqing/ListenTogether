...
## 本轮新增（LOAD-15 云端公网重测，2026-09-23 晚）

- **M4 最后一个量化门槛关闭**（W4/T4）：15 路 × 600 秒 playback 模型经公网直连 `http://8.166.126.136:3000`，**全 206（8907 请求）、零失败、总吞吐 2.847 Mbps**（目标 2.88 的 98.9%，为本地基线 2.873 的 99.1%）；每路平均速率 23,559–23,879 B/s 全部 ≥ 目标×0.9；每路 1 次读取取消 + 半程随机跳转/尾回绕（seeks 28）覆盖 Range 跳转与取消路径。60 秒试跑先行确认无 401/404 后再进正式轮（C6 流量纪律）。记录见 [test-results/2026-09-23-load15-cloud](test-results/2026-09-23-load15-cloud/README.md)。
- 前置曲库：`demo-load`（合成 11 分钟 192kbps）上传云端曲库并重启生效，云端曲库现为 **6 首**（5 首真实 MP3 + demo-load）；demo-load 按计划保留（后续测试可复用）。上传过程踩到新坑（本会话沙箱内 PowerShell 通道 scp 报 exit 255 "pipe: Unknown error"，Bash 通道正常）已回填 [陷阱清单第 7 节](development-pitfalls.md)。
- 执行纪律：C1 时间片声明/释放（23:2x 声明 → 23:51 释放，与升级/回滚演练时间片错开）；正式轮期间仅 3 次轻量 SSH 观察（C7），内存全程 ≥1175MB available、journal 零 error/warn、NRestarts=0、PID 未变；公网出口实测出网 ≈3.16Mbps（净 2.88 + 开销），**带宽未成瓶颈，未出现降速形态**。
- 证据边界（如实标注）：服务端负载模型（无 APK/真机参与）；1 秒粒度存在"请求跨秒边界切分"型部分字节秒（最小 103B，无一路跌破 90% 平均门槛，但连续低速秒无法从汇总排除）；throughput（无节流）模型未在云端执行（流量预算外，维持标注）。
- 流量实测：本轮出网约 **235MB**（试跑 21.2 + 正式 213.5 + 杂项），远低于此前 2.2GB 的估算口径（该估算高了一个数量级，192kbps×15×600s=216MB 才是正确口径）；月 20GiB 额度约束比预期更宽松。
- 本轮未改产品代码、未重建 APK（hash 仍 36BD3A5B…）。

## 本轮新增（收尾汇总：全量回归 + 文档一致性 + 入口快照，2026-09-24）

> 收尾会话：不改产品代码、不动服务器；对 W1–W4 并行交付做统一回归、一致性核对与入口同步。

- **全量回归通过**（scripts/check.ps1 -Scope all）：后端 tsc 0 错误 + **7/7 测试**；安卓 testDebugUnitTest（输入未变 UP-TO-DATE，**45 项**）+ assembleDebug + lintDebug 通过（BUILD SUCCESSFUL 52s，**Lint 0 错误 0 警告**）；Markdown 本地链接检查通过。无环境问题、无真实失败。
- **APK hash 不变声明**：本轮未改产品代码、未重建 APK，交付锚定 **36BD3A5B3ACA74EE45CCEE952F6EB8C042BB0A18D40123601398ADF626DD0CCE**；对本文件全量检索核对，全部 SHA256 均为既有历史记录，W1–W4 各节引用一致，**无并行会话引入新 hash**。
- **文档一致性核对与最小合并**：「尚待验收」清单勾选与正文逐条核对一致；唯一矛盾——LOAD-15 条目（2026-09-22 行）尾巴"云端 TLS/公网重测仍属 M4 门槛"已被 2026-09-23 晚云端通过推翻，已就地更正并指向完成记录（不重写他人结论）。test-results 链接经 check-doc-links.mjs 全部存在。
- **入口快照同步**：AGENTS.md 快照更新为"2026-09-24 收尾确认"并新增"下一步"结构（剩余待办 = M2 双机（缺设备）/ M3-LONG（挂起）/ TLS 正式化（用户决策路线 A），另列条件具备时的补测项）；[parallel-development-plan.md](parallel-development-plan.md) W1/W2 小节与执行顺序表标注已完成（W3/W4 此前已标），T1/T2 清零，模块快照表（01/03/04/08）同步为验收后状态；同文两处过时口径一并更正——LOAD-15 云端流量"2.2GB 估算"改为**实测 ≈235MB/轮**、"36BD3A5B 未真机验收"改为已验收并完成公网 E2E。
- **工作区清理**：[交接单](handover-2026-09-23.md)第七节列出的临时产物已删除（`.workbuddy/tmp-diag-*.jsonl` × 4、services-dump.txt、flinger-a.txt、storm-window.txt、sync-offsets.txt，共 8 个）；.workbuddy/memory、诊断分析脚本、w1_driver.py 可复跑实现与 W1/W2 过程证据文件均未触碰。
- **M4 门槛关闭清单**（汇总引用，结论见上文各节）：①首次部署 + 13 项服务端验证 + SSH 隧道联调（2026-09-22）；②真机公网 E2E 建房→播放全链路（W2，APK 36BD3A5B…）；③升级/回滚演练双向通过（W3，13 项抽查三次各 13/0）；④LOAD-15 云端公网重测（W4，15 路×600s 全 206 零失败、2.847Mbps=本地基线 99.1%）。
- 本轮边界：纯收尾交付——未执行真机/云端测试，未改任何产品代码、脚本与服务器状态；云端入口、曲库与 release 链接维持 2026-09-23 晚状态（在产 20260922-2159）。
...
- [x] LOAD-15：本地 15 路负载 10 分钟记录通过（2026-09-22，见 test-results/2026-09-22-load15）；云端公网重测已于 2026-09-23 晚通过，详见下一条（原"云端 TLS/公网重测仍属 M4 门槛"的过时表述已随 M4 关闭更正）。
- [x] LOAD-15 完成（本地）；15 路真实音频带宽在云端公网重测**通过**（2026-09-23 晚，IP 明文入口，15 路×600s 全 206 零失败、2.847Mbps=基线 99.1%，见 [test-results/2026-09-23-load15-cloud](test-results/2026-09-23-load15-cloud/README.md)；throughput 模型与 TLS 条件未测，TLS 按路线 A 决策维持明文）。
...
- [x] 15 路实际音频带宽在云端重测（LOAD-15 云端部分，归 M4；**2026-09-23 晚通过**，见上两行与 [test-results/2026-09-23-load15-cloud](test-results/2026-09-23-load15-cloud/README.md)）。**M4 四项部署门槛（部署验证/公网 E2E/升级回滚/15 路云端重测）至此全部关闭**。
