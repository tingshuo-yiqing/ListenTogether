...
- **房主生命周期**：全员超时清扫时清空旧 `hostId`；无现存房主时 connect 在首份在线快照前补位，避免到下一 tick 才有控制权限。60 秒宽限、最早在线成员与五分钟回收规则不变。**仅本地修复，未部署云端**。
...
|---|---|---|---|---|
| EC5FCF0A987D10087CE88CEC228CF4509CDCC5B4E6D06E0CC79662005A4C4870 | 09-26 | 遗留修复：邀请提示/解析边界、实际播放倍速复位 | 91 | 未装机；Lint 0，见 legacy-fixes |
...
## 尚待真机与云端验收
- [ ] 09-26 遗留修复终稿（debug `EC5FCF0A…` / benchmark `F943E3CA…`）未装机；倍速实际复位与邀请入口需真机回归，原扫码/布局/M2 等挂起项继续保留。
- [ ] 09-26 房主清扫与 connect 补位修复待云端部署；本地 23 项后端测试通过，线上仍为此前版本。
- [ ] 09-26 终稿（debug `BCF3DE16…` / benchmark `9E17F291…`）：**未装机目视**（用户此后无法提供真机）。改动为纯删除文案与参数收敛，门禁 84 项单测 + Lint 0；装机后需目视：MiniPlayer 只剩歌名、展开页只剩标题/主控/进度/时间、异常与本机暂停仍由横幅承载。
