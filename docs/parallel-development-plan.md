...
|---|---|---|
| 01 Android UI | 已实现；简洁 UI + 优化批次真机验证通过；卡顿修复版（36BD3A5B）进度条端点已真机目视通过（W1） | [ui-refresh](test-results/2026-09-23-ui-refresh/README.md)、[ui-optimize](test-results/2026-09-23-ui-optimize/README.md)、[w1-recheck](test-results/2026-09-23-w1-recheck/README.md) |
| 02 网络与会话 | 已实现并真机验证（状态机/代次隔离/重连/401）；无待办改动 | m1/m3-auth-recheck 记录 |
| 03 播放服务 | 分级纠正 + SmoothRenderers 真机验收通过（W1：关省电 seek=0、开省电变速追赶） | [w1-recheck](test-results/2026-09-23-w1-recheck/README.md) |
| 04 同步算法 | SyncMath 分级纠正已落地（45 项单测通过）；`speed` 变速追赶真机实测确认（W1：media_session speed 1.04→1.00） | SyncMathTest 3 组新用例、[w1-recheck](test-results/2026-09-23-w1-recheck/README.md) |
| 05/06/07 后端房间/HTTP/WS | 已实现并稳定；本轮无必须改动 | 首次部署 13/13 |
| 08 曲库与音频 | 已实现；云端 6 首（5 首真实 192k + demo-load 负载测试音）；曲库工具链就绪 | deployment.md 第 6 节 |
| 09 构建/部署 | 云端单实例运行中（0.0.0.0:3000，release 20260922-2159）；**升级/回滚演练已完成**（2026-09-23 晚，双向通过，新 release 20260923-2157 留作真实升级候选） | [m4-first-deploy](test-results/2026-09-22-m4-first-deploy/README.md)、[m4-rollback-drill](test-results/2026-09-23-m4-rollback-drill/README.md) |
...
| 编号 | 事项 | 状态 |
|---|---|---|
| T1 | 卡顿修复（36BD3A5B）真机验收，5 项清单 | **已完成**（2026-09-23 晚 W1，见 [w1-recheck](test-results/2026-09-23-w1-recheck/README.md)） |
| T2 | M4 真机公网 E2E（建房→播放） | **已完成**（2026-09-23 晚 W2，见 [m4-public-e2e](test-results/2026-09-23-m4-public-e2e/README.md)） |
| T3 | M4 升级/回滚演练 | **已完成**（2026-09-23 晚：升级到 20260923-2157 → 回滚到 20260922-2159，两次 health 200 + 13 项抽查各 13/0；时间片已释放） |
| T4 | LOAD-15 云端公网重测 | **已完成**（2026-09-23 晚：15 路×600s 全 206 零失败 2.847Mbps；demo-load 保留云端曲库） |
...
| 公网出网流量 | 大陆地域免费 20GiB/月；LOAD-15 云端**实测 ≈235MB/轮**（旧估算 2.2GB 高一个数量级；正确口径 192k×15×600s=216MB） | working memory/verification |
| APK 版本 | 当前锚 **36BD3A5B（45 单测）已真机验收（W1）并完成公网 E2E（W2）**；后续改码即换锚，真机/公网结论必须与 hash 绑定 | verification.md |
...
### W1 · 真机验收批次（P0，串行占用真机；**2026-09-23 晚已完成**）

> **状态：完成（通过）**，记录见 [w1-recheck](test-results/2026-09-23-w1-recheck/README.md)：关省电 180s 位置推进 1.001x 且诊断 `seek=0`、开省电 `speed` 变速追赶 9 条且 `seek` 仍 0、自动切歌两段零 seek、进度条端点 14dp 圆点/5dp 轨道像素实测、暗色冷启动无白闪、听感由用户确认连续。执行前纠正了装机偏差（设备实为 169018AE，已重装 36BD3A5B 并回拉复算锚定）。**下面保留原任务定义以备复用。**
...
### W2 · M4 公网 E2E（P0，依赖 W1；**2026-09-23 晚已完成**）

> **状态：完成（通过）**，记录见 [m4-public-e2e](test-results/2026-09-23-m4-public-e2e/README.md)：同一 APK（36BD3A5B）经 `http://8.166.126.136:3000` 完成 建房→选歌→播放→暂停→拖动→切歌→退出 全链路（房间 version 2→9），公网校时 RTT 中位 65ms，诊断 `seek` 仅大跨度拖动/切歌归零 2 条、切歌后新段 `seek=0`；第二房间复测通过。**下面保留原任务定义以备复用。**
...
### W3 · M4 升级/回滚演练（P1，两段式；**2026-09-23 晚已完成**）

> **状态：完成（升级成功 + 回滚成功）**，记录见 [m4-rollback-drill](test-results/2026-09-23-m4-rollback-drill/README.md)。
> 段 1 于 21:57 与真机轨道并行产出 `listen-together-0.1.0-20260923-2157.tar.gz`（34/34 校验、与在产包 `server/` 哈希全等）；
> 段 2 于 22:52–22:57 独占云端时间片完成升级与真实回滚（两次 health 第 2 秒 200、PID 4191→14935→15175、13 项抽查各 13/0）。
> 两次 `restart` 各清空一次内存房间，已用受控房间实测 `catalog 200 → 404`；执行前确认无活跃房间，未影响真实会话。
> 新 release `20260923-2157` 保留在服务器作为将来真实升级的候选。**下面保留原任务定义以备复用。**
...
### W4 · LOAD-15 云端公网重测（P1，依赖 W2；**2026-09-23 晚已完成**）

> **状态：完成（通过）**，记录见 [load15-cloud](test-results/2026-09-23-load15-cloud/README.md)。15 路 × 600s 公网直连全 206、零失败、2.847Mbps（本地基线的 99.1%）；demo-load 已上传云端曲库并保留；C1 时间片已释放。**下面保留原任务定义以备复用。**
...
|---|---|---|---|---|---|
| 1 | **W1 真机验收批次** | P0 | 无（条件：无线 adb + 关省电） | 真机 30–40 分钟 | ✅ **已完成**（2026-09-23 晚，见 [w1-recheck](test-results/2026-09-23-w1-recheck/README.md)） |
| 2 | **W2 公网 E2E** | P0 | W1 通过；云端可达 | 真机 15 分钟 + 记录 | ✅ **已完成**（2026-09-23 晚，见 [m4-public-e2e](test-results/2026-09-23-m4-public-e2e/README.md)） |
| 3 | W3 本地段：打包（可与 1/2 并行） | P1 | 无 | 本地 5 分钟 | 新 tar.gz + SHA256 清单就绪 |
| 4 | **W3 云端段：升级+回滚演练** | P1 | 本地段；云端时间片 | 独占服务 ~20 分钟（实测 ~5 分钟） | ✅ **已完成**（2026-09-23 晚）：升级成功 + 回滚到 20260922-2159 成功 + 两次 health 复核 + 13 项抽查各 13/0 |
...
**【2026-09-24 收尾确认】上述最短路径已于 2026-09-23 晚全部走完**：W1/W2/W3 云端段/W4 均完成并通过，M4 四项门槛关闭，T1–T4 清零；剩余待办即 W5（M2 双机，缺设备）/ W6（M3-LONG，挂起）/ W7（TLS 正式化，用户决策路线 A），项目转入试用反馈驱动的修复循环或 0.2.0 收尾。
